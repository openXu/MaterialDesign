...
import static com.openxu.md.view.chart.BaseChart.TOUCH_EVENT_TYPE.EVENT_X;
...
public class BarVerticalChart extends BaseChart{
    ...
    private int barWidth = FConversUtils.dip2px(getContext(), 15);    //柱宽度
    private int barSpace = FConversUtils.dip2px(getContext(), 1);    //双柱间的间距
    private int barItemSpace = FConversUtils.dip2px(getContext(), 25);//一组柱之间的间距
    ...
    private int textSpace = FConversUtils.dip2px(getContext(), 3);         //默认的字与其他的间距
    private int textLableSpace = FConversUtils.dip2px(getContext(), 10);   //默认的lable字与其他的间距
    private int lableItemSpace = FConversUtils.dip2px(getContext(), 30);   //默认的lable字与其他的间距
    private int lableTopSpace = FConversUtils.dip2px(getContext(), 20);   //默认的lable字与其他的间距
    ...
    private void evaluatorByData() {
        ...
        FLog.i(TAG, "真实YMARK_MAX=" + YMARK_MAX);
        if (YMARK_MAX <= 5)
            YMARK_MAX = 5;
        ...
        FLog.i(TAG, "计算YMARK_MAX=" + YMARK_MAX + "   YMARK=" + YMARK);
        //Y轴刻度间距值
        YMARK_ALL_H = (int) (zeroPoint.y - getPaddingTop()  - textSpace - heightTag);/*- heightCoordinate*/
        YMARK_H = (int) ((float) YMARK_ALL_H / YMARK_NUM);
        FLog.i(TAG, "zeroPoint.y=" + zeroPoint.y + "   textSpace=" + textSpace+"  heightTag="+heightTag+"   heightCoordinate="+heightCoordinate);
        //单份柱子（包含多个及其间距）宽度
        ...
        FLog.w(TAG, "柱状图表宽高：" + getMeasuredWidth() + "*" + getMeasuredHeight() +
                "  图表范围" + rectChart + "   圆点坐标zeroPoint=" + zeroPoint);
        FLog.w(TAG, "YMARK_MAX=" + YMARK_MAX + "   YMARK=" + YMARK + "  YMARK_H=" + YMARK_H + "   YMARK_MAX_WIDTH=" + YMARK_MAX_WIDTH);
        FLog.w(TAG, "minLeftPointX=" + minLeftPointX + "   mMoveLen=" + mMoveLen + "  leftStartPointX=" + leftStartPointX);
    }
...
//            FLog.w(TAG, "绘制直线start="+startX+"*"+startY+"  stop="+stopX+"*"+stopY);
...
//        FLog.i(TAG, "leftStart："+leftStart+"   zezeroPoint"+zeroPoint+"   barItemSpace="+barItemSpace+"   barWidth="+barWidth +"   oneBarW="+oneBarW);
...
//            FLog.i(TAG, "绘制X刻度：leftX="+leftX   +"    "+(leftX + oneBarW/2-tw/2)+"*"+(zeroPoint.y+textSpace + leadCoordinate));
//            FLog.i(TAG, "leftStartPointX="+leftStartPointX+"  leftStart="+leftStart+ "   oneBarW="+oneBarW   +"    barWidth="+barWidth+"   barSpace="+barSpace);
            paintLabel.setTextSize(textSizeTag);
...
//                FLog.d(TAG, "绘制bar："+(br)+"    "+getMeasuredWidth()+"*"+getMeasuredHeight());
...
//        FLog.d(TAG, "遮盖超出的柱状："+new RectF(0,0,zeroPoint.x, lableRect.top));
//        FLog.d(TAG, "遮盖超出的柱状："+new RectF(rectChart.right,0,getMeasuredWidth(), lableRect.top));
    ...
    protected void consumeBySelf(int dx, int dy, int[] consumedBySelf) {
        consumedBySelf[1] = 0; //y轴不消耗
        if (leftStartPointX + mMoveLen + dx <= minLeftPointX) {
            //向左滑到达最右侧 dx<0
            consumedBySelf[0] = minLeftPointX - leftStartPointX - mMoveLen;
            mMoveLen = mMoveLen + consumedBySelf[0];
